...
Jac1=zeros(m,q);
l=n-(length(alpha)-1); %degree of the B-splines
...
% for j=1:l+1,                          % for each nonlinear parameter alpha(j)
%     range = find(Ind(2,:)==j);        % columns of WdPhi relevant to alpha(j)
%     indrows = Ind(1,range);           % relevant rows of ctemp
%     nonzeroInd=1:floor(alpha(j+l+1));
%     Jac1a(nonzeroInd,j) = WdPhi(nonzeroInd,range) * ctemp(indrows);    
%     T2(indrows,j) = WdPhi_r(range);
% end
% 
% for j=l+2:q-l-2,                      % for each nonlinear parameter alpha(j)
%     range = find(Ind(2,:)==j);        % columns of WdPhi relevant to alpha(j)
%     indrows = Ind(1,range);           % relevant rows of ctemp
%     nonzeroInd=floor(alpha(j-l-1)):floor(alpha(j+l+1));    
%     Jac1a(nonzeroInd,j) = WdPhi(nonzeroInd,range) * ctemp(indrows);
%     T2(indrows,j) = WdPhi_r(range);
% end
% 
% for j=q-l-1:q,                          % for each nonlinear parameter alpha(j)
%     range = find(Ind(2,:)==j);        % columns of WdPhi relevant to alpha(j)
%     indrows = Ind(1,range);           % relevant rows of ctemp
%     nonzeroInd=floor(alpha(j-l-1)):m;    
%     Jac1a(nonzeroInd,j) = WdPhi(nonzeroInd,range) * ctemp(indrows);    
% %     subplot(1,2,1);
% %     spy(WdPhi(floor(alpha(j-l-1)):end,range),'b');
% %     subplot(1,2,2);
% %     spy(WdPhi(:,range),'r');
%     T2(indrows,j) = WdPhi_r(range);
% end

Jac1 = Jac1 - U(:,1:myrank) * (U(:,1:myrank)' * Jac1);
